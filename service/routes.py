...
def create_counters(name):
    ...
    if name in COUNTER: return abort(status.HTTP_409_CONFLICT, f"Counter {name} already exists")
...
def read_counters(name):
    ...
    if name not in COUNTER: return abort(status.HTTP_404_NOT_FOUND, f"Counter {name} does not exist")
